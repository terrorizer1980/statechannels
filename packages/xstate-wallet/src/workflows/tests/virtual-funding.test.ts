...
const outcome: Outcome = simpleEthAllocation([
  ...
  {destination: jointParticipants[ParticipantIdx.Hub].destination, amount: amounts.reduce(BN.add)},
...
const depositAmount = ledgerAmounts.reduce(BN.add);
      ...
      simpleEthAllocation([
        {
          ...
          amount: BN.sub(ledgerAmounts[0], amounts[0])
        },
        {
          destination: jointParticipants[ParticipantIdx.Hub].destination,
          amount: BN.sub(ledgerAmounts[1], amounts[1])
        },
        // We don't know the guarantor channel id
        {destination: expect.any(String), amount: amounts.reduce(BN.add)}
      ])
    );
