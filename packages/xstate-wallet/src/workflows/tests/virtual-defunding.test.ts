...
const totalTargetAmount = targetAmounts.reduce(BN.add);
...
async function expectBudgetIsUpdated(expectedLedgerAmounts: Uint256[], store: TestStore) {
  ...
  expect(ethBudget.availableSendCapacity).toEqual(expectedLedgerAmounts[1]);
  expect(ethBudget.availableReceiveCapacity).toEqual(expectedLedgerAmounts[0]);
