...
const totalAmount = amounts.reduce(BN.add);
...
const ledgerAmounts = amounts.map(a => BN.add(a, 2));
...
test('it uses direct funding when told', async () => {
  ...
  expect((await aStore.chain.getChainInfo(targetChannelId)).amount).toEqual(totalAmount);
});
